/**
 * List Chapter 14 Trust Accounts headings from PUB725 book + DB.
 * No chapter-folder notes PDF for Chapter 14.
 * Run: PATH=./node_modules/.bin:$PATH tsx --env-file=.env scripts/list-ch14-headings.ts
 */
...
const BOOK_PDF_START = 243;
const BOOK_PDF_END = 248; // course book ends ~248
...
async function main() {
  ...
  const ch14 = book.headings.filter(
    (h) => h.pdfPage >= BOOK_PDF_START && h.pdfPage <= BOOK_PDF_END,
  );
  console.log(`=== BOOK Ch14 headings (pdf ${BOOK_PDF_START}-${BOOK_PDF_END}) ===`);
  for (const h of ch14) {
  ...
  const sections = await prisma.sourceSection.findMany({
    where: { chapterNumber: 14, NOT: { heading: { contains: "(cont.)" } } },
    select: {
      heading: true,
      pdfPageStart: true,
  ...
  console.log("\n=== DB chapterNumber=14 sections (no cont.) ===");
