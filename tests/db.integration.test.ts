...
describe("database seed (integration)", () => {
  ...
  it("seeds Chapter 1–6 concepts and active confusion pairs idempotently", async ({ skip }) => {
    if (!(await databaseIsReachable())) {
      skip();
      return;
    }
    ...
    expect(
      await prisma.concept.count({ where: { editionId: edition.id, chapterNumber: 6 } }),
    ).toBe(CHAPTER_6_CONCEPTS.length);
  ...
  it("seeds Ch1–Ch6 assets and advances sittings only after prior chapters complete", async ({
    skip,
  }) => {
    if (!(await databaseIsReachable())) {
      skip();
