...
This repository is **Phase 4**: sourced Teach Me sittings for **Chapters 1–8**, on top of ingestion and the concept graph. The product goal is all **14 chapters**. Quizzes and class-miss photos are not in yet.
...
The second `npx prisma db seed` attaches Chapter 1–8 concepts and Teach Me assets to ingested section IDs. `npm run ingest` also re-seeds those after writing sections (PUB725 + Chapter 1–8 notes).
...
- Hand-authored Chapter 1–8 learning assets (explanation, simple language, scenario, comparison, teach-back, recall) with heading citations
- Sequential Teach Me planners through Chapter 8 Title of Real Estate (`phase4-ch8-v1`)
- Start session / Resume on the local database (`/` stays static for Vercel)
- Progress lists Chapters 1–8 and may mark taught concepts as learning — still no 0% bars
- Architecture lock in [docs/decisions.md](docs/decisions.md)

## What Phase 4 does not include

Quizzes, class-miss photo intake, tutor, math solver, exam simulator, embeddings, OpenAI, OCR, video, XP, authentication, Teach Me sittings for chapters 9–14 (same loop comes next).
...
- Ingest CLI for PUB725 + Chapter 1–8 notes
