/**
 * List Chapter 13 Contract Law headings from PUB725 book + DB.
 * No chapter-folder notes PDF for Chapter 13.
 * Run: PATH=./node_modules/.bin:$PATH tsx --env-file=.env scripts/list-ch13-headings.ts
 */
...
const BOOK_PDF_START = 233;
const BOOK_PDF_END = 242; // Ch14 starts at pdf 243
...
async function main() {
  ...
  const ch13 = book.headings.filter(
    (h) => h.pdfPage >= BOOK_PDF_START && h.pdfPage <= BOOK_PDF_END,
  );
  console.log(`=== BOOK Ch13 headings (pdf ${BOOK_PDF_START}-${BOOK_PDF_END}) ===`);
  for (const h of ch13) {
  ...
  const sections = await prisma.sourceSection.findMany({
    where: { chapterNumber: 13, NOT: { heading: { contains: "(cont.)" } } },
    select: {
      heading: true,
      pdfPageStart: true,
  ...
  console.log("\n=== DB chapterNumber=13 sections (no cont.) ===");
