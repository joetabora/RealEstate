...
describe("database seed (integration)", () => {
  ...
  it("seeds Chapter 1–5 concepts and active confusion pairs idempotently", async ({ skip }) => {
    if (!(await databaseIsReachable())) {
      skip();
      return;
    }
    ...
    expect(
      await prisma.concept.count({ where: { editionId: edition.id, chapterNumber: 5 } }),
    ).toBe(CHAPTER_5_CONCEPTS.length);
  ...
  it("seeds Ch1–Ch5 assets and advances sittings only after prior chapters complete", async ({
    skip,
  }) => {
    if (!(await databaseIsReachable())) {
      skip();
