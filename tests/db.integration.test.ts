...
describe("database seed (integration)", () => {
  ...
  it("seeds Chapter 1–10 practice questions and records attempts with knowledge vs performance", async ({
    skip,
  }) => {
    if (!(await databaseIsReachable())) {
      skip();
    ...
    expect(
      await prisma.question.count({
        where: { editionId: edition.id, chapterNumber: 10, lifecycle: "active" },
      }),
    ).toBe(PHASE5_CH10_QUESTIONS.length);
    ...
    const session = await startOrResumeChapterPractice(10);
