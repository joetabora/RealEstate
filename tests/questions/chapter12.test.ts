...
import { PHASE5_CH12_QUESTIONS } from "@/lib/questions/chapter12";
import {
  citationsForQuestion,
  pairKeyForQuestion,
} from "@/lib/questions/catalog";
import { CHAPTER_12_CONFUSION_PAIRS, canonicalPairKey } from "@/lib/knowledge";

describe("Phase 5 Chapter 12 questions", () => {
  it("covers every Chapter 12 confusion pair with citations and one correct option", () => {
    expect(PHASE5_CH12_QUESTIONS).toHaveLength(CHAPTER_12_CONFUSION_PAIRS.length);
    for (const question of PHASE5_CH12_QUESTIONS) {
      expect(question.chapterNumber).toBe(12);
  ...
  it("only uses active Chapter 12 confusion pairs", () => {
  ...
  it("does not invent Wisconsin statute numbers or dollar fees", () => {
    const text = PHASE5_CH12_QUESTIONS.map(
      (question) =>
